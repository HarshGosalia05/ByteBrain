...
class M2V2Predictor:
    ...
    def _validate_artifact(self) -> None:
        ...
        required_keys = ("models", "preprocessor", "feature_names", "metadata")
        for key in required_keys:
            if key not in self._artifact:
                raise ValueError(
                    f"M2 v2 artifact is missing required key '{key}'. "
                    "The artifact may be corrupted or from an incompatible version."
                )
        metadata = self._metadata
        if not isinstance(metadata, dict):
            raise ValueError(
                "M2 v2 artifact metadata is not a dict. "
                "The artifact may be corrupted or from an incompatible version."
            )
        ...
        if version is not None:
            try:
                ...
                if major < 2:
                    raise ValueError(
                        f"M2 v2 artifact version {version} is incompatible "
