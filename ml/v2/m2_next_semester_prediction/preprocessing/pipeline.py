"""M2 v2 — Preprocessing Pipeline.

Shared numeric feature matrix `X` (used identically for both regression
targets, `next_semester_sgpa` and `next_semester_percentage`).

Encoding (stateless — no fit needed):
    - gender          -> is_male (0/1)
    - stress level    -> stress_ordinal (Low=0, Medium=1, High=2)

`select_features` NEVER includes a target or any forbidden T+1 column.
Imputation is handled by `M2Preprocessor`, fit on training folds only.
"""
...
def select_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    target columns carried alongside. Output: only allowed feature columns.
    """
    work = df.copy()
    ...
    # Final leakage check (must contain neither target nor any T+1 outcome)
    all_forbidden = set(config.FORBIDDEN_FEATURES) | set(config.TARGETS)
    leaked = [c for c in all_forbidden if c in X.columns]
    if leaked:
        raise ValueError(f"M2 leakage in feature matrix: {leaked}")
...
class M2Preprocessor:
    ...
    def transform(self, X: pd.DataFrame) -> np.ndarray:
        if not self._is_fitted:
            raise RuntimeError("M2Preprocessor has not been fitted yet.")
