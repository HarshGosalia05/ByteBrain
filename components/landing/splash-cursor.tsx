...
export function SplashCursor({
  ...
  React.useEffect(() => {
    ...
    function getWebGLContext(targetCanvas: HTMLCanvasElement) {
      ...
      let webglContext: WebGLRenderingContext | WebGL2RenderingContext | null = targetCanvas.getContext("webgl2", params)
      const isWebGL2 = Boolean(webglContext)
      if (!isWebGL2) {
        webglContext = (targetCanvas.getContext("webgl", params) ||
      ...
      if (isWebGL2) {
        ...
        formatRGBA = getSupportedFormat(webglContext, gl2.RGBA16F, gl2.RGBA, halfFloatTexType!)
        formatRG = getSupportedFormat(webglContext, gl2.RG16F, gl2.RG, halfFloatTexType!)
        formatR = getSupportedFormat(webglContext, gl2.R16F, gl2.RED, halfFloatTexType!)
      } else {
    ...
    function getSupportedFormat(
      ...
      if (!supportRenderTextureFormat(webgl, internalFormat, format, type)) {
        ...
        switch (internalFormat) {
          case gl2.R16F:
            return getSupportedFormat(webgl, gl2.RG16F, gl2.RG, type)
          case gl2.RG16F:
            return getSupportedFormat(webgl, gl2.RGBA16F, gl2.RGBA, type)
          default:
            return null
        }
      }
    ...
    class Material {
      ...
      setKeywords(keywords: string[]) {
        ...
        if (program == null) {
          ...
          const createdProg = createProgram(this.vertexShader, fragmentShader)
          if (!createdProg) return
          program = createdProg
    ...
    function compileShader(type: number, source: string, keywords?: string[] | null): WebGLShader | null {
      const formattedSource = addKeywords(source, keywords)
      const shader = gl.createShader(type)
      if (!shader) return null
      gl.shaderSource(shader, formattedSource)
    ...
    function clickSplat(pointer: Pointer) {
      ...
      // Subdued intensity for subtle ambient burst
      color.r *= 1.5
      color.g *= 1.5
      color.b *= 1.5
    ...
    function hexToRGB(hex: string) {
      ...
      // Ambient multiplier: tuned for smooth, subtle visibility without overpowering text
      return { r: r * 0.14, g: g * 0.14, b: b * 0.14 }
    }
