...
export function M3FacultyReview({ studentId }: { studentId: string }) {
  ...
  if (loadError) {
    return <p className="text-sm text-muted-foreground">Faculty review is unavailable right now.</p>
  }
    ...
    <div className="flex flex-col gap-3 rounded-lg border border-border bg-muted/30 p-3">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        >
          ...
          {verdict?.feedback_action === "confirmed" ? "Confirmed" : "Confirm prediction"}
        </Button>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          {verdict?.feedback_action === "dismissed" ? "Dismissed" : "Dismiss prediction"}
        </Button>
      </div>
