...
import type { WorkloadHighlight } from "@/lib/faculty-api"
import type { SectionResult } from "@/lib/section-result"

type HighlightsResult = SectionResult<{ items: WorkloadHighlight[] }>

function severityStyle(severity: WorkloadHighlight["severity"]) {
  switch (severity) {
    case "warning":
      return {
        icon: TriangleAlert,
...
export function HighlightsView({ data }: { data: HighlightsResult }) {
  ...
  const handleHighlightClick = (highlight: WorkloadHighlight) => {
    ...
    <section id="highlights" className="flex scroll-mt-6 flex-col gap-4">
      ...
      {data.error ? (
                  ...
                  <span className="min-w-0 flex-1">
                    ...
                    {(highlight.subject_code || highlight.term_label) && (
                      <span className="mt-1 block text-xs text-muted-foreground">
                        {[highlight.subject_code, highlight.term_label].filter(Boolean).join(" · ")}
        ...
        <div className="flex items-center gap-3 rounded-xl bg-card p-4 ring-1 ring-foreground/10">
          ...
          <p className="text-sm text-muted-foreground">
            Your workload is balanced within the configured capacity baselines.
          </p>
        </div>
      )}
    </section>
