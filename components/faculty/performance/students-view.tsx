...
const SORTABLE: Record<string, { label: string; align?: "right" }> = {
  enrollment_no: { label: "Enrollment No" },
  name: { label: "Student Name" },
  semester: { label: "Sem" },
  subject: { label: "Subject" },
  attendance: { label: "Attendance" },
  total_marks: { label: "Performance", align: "right" },
  grade: { label: "Grade" },
  result_status: { label: "Result" },
}
...
function gapStatusBadge(status: string) {
  switch (status) {
    ...
    case "On Track":
      return (
        <span className="inline-flex items-center rounded-full bg-chart-2/15 px-2.5 py-0.5 text-xs font-medium text-chart-2">
          {status}
        </span>
      )
    default:
...
export function StudentsView({
  ...
  data: SectionResult<PerformanceStudentsResponse>
  ...
  const handleExport = async (ids: string[]) => {
    ...
    try {
      ...
      const res = await exportPerformanceCsvAction({
      ...
      if (res.ok) {
        downloadCsv(res.data.data, `faculty_performance_${scope}_${Date.now()}.csv`)
      } else {
    ...
    <section id="students" className="flex scroll-mt-6 flex-col gap-4">
      ...
      <div className="flex flex-wrap items-center gap-3 print:hidden">
        ...
        <select
          ...
          value={gapStatus}
          onChange={(e) => handleFilterChange("gap_status", e.target.value)}
          aria-label="Learning-gap status"
        >
          <option value="">All gap statuses</option>
          <option value="Below Baseline">Below Baseline</option>
          ...
          <TableHeader>
            <TableRow>
              ...
              {Object.entries(SORTABLE).map(([key, col]) => {
                ...
                return (
                  <TableHead key={key} className={col.align === "right" ? "text-right" : undefined}>
                    <button
                      type="button"
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row) => (
                ...
                >
                  ...
                  <TableCell>{row.result_status ?? "—"}</TableCell>
                  <TableCell>{gapStatusBadge(row.gap_status)}</TableCell>
                </TableRow>
              ))
            )}
          </TableBody>
