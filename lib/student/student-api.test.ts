...
// The per-student route /predict/m1v3/{student_id} is consumed by the student
// BFF. NO_DATA is returned as 200 with readiness_status="NO_DATA" and a reason;
// READY returns per-subject predicted end-sem marks on the /70 scale.
// ---------------------------------------------------------------------------
...
const M1V3_BODY = {
  ...
  note: "Predicted end-sem marks are model estimates, not actual results. This prediction was generated by the M1 V3 clean model trained on real CampusX data. Uncertainty is unavailable for this model.",
}

test("getStudentM1V3 fetches /predict/m1v3 with own student id and parses typed result", async () => {
  route("/predict/m1v3/STU-A", 200, M1V3_BODY)

  const result = await studentApi.getStudentM1V3()
  assert.equal(result.ok, true)
  if (!result.ok) return
  assert.equal(result.data.model_id, "m1_v3")
  assert.equal(result.data.model_version, "m1_v3_clean")
  assert.equal(result.data.readiness_status, "READY")
  assert.equal(result.data.subjects.length, 1)
  const subject = result.data.subjects[0]
  assert.equal(subject.subject_name, "Data Structures")
  assert.equal(subject.predicted_end_sem_marks, 55.5)
  assert.equal(subject.grade_band, "A")
  assert.equal(subject.input_features.attendance_percentage, null)
  assert.ok(subject.predicted_end_sem_marks <= subject.target_max)

  const hit = getCalls("/predict/m1v3")
  assert.equal(hit.length, 1)
  assert.equal(hit[0].url, "http://localhost:8000/api/v1/predict/m1v3/STU-A")
})

test("getStudentM1V3 url-encodes the student id", async () => {
  activeSession = { ...DEFAULT_SESSION, student_id: "STU A/1" }
  route("/predict/m1v3/STU%20A%2F1", 200, M1V3_BODY)

  const result = await studentApi.getStudentM1V3()
  assert.equal(result.ok, true)
  const hit = getCalls("/predict/m1v3")
  assert.equal(hit.length, 1)
  assert.equal(hit[0].url, "http://localhost:8000/api/v1/predict/m1v3/STU%20A%2F1")
})

test("getStudentM1V3 auth gating (401 / 403 / 400) applies", async () => {
  activeSession = null
  const noSession = await studentApi.getStudentM1V3()
  assert.equal(noSession.ok, false)
  if (!noSession.ok) assert.equal(noSession.error.status, 401)

  activeSession = { ...DEFAULT_SESSION, role: "Faculty" }
  const wrongRole = await studentApi.getStudentM1V3()
  assert.equal(wrongRole.ok, false)
  if (!wrongRole.ok) assert.equal(wrongRole.error.status, 403)

  activeSession = { ...DEFAULT_SESSION, student_id: null }
  const unlinked = await studentApi.getStudentM1V3()
  assert.equal(unlinked.ok, false)
  if (!unlinked.ok) assert.equal(unlinked.error.status, 400)
})

test("getStudentM1V3 returns 200 with NO_DATA for unavailable predictions", async () => {
  const NO_DATA_BODY = {
    ...M1V3_BODY,
    readiness_status: "NO_DATA",
    reason: "Required academic data is missing for the current semester.",
    subjects: [],
    prediction_count: 0,
  }
  route("/predict/m1v3/STU-A", 200, NO_DATA_BODY)
  const result = await studentApi.getStudentM1V3()
  assert.equal(result.ok, true)
  if (result.ok) {
    assert.equal(result.data.readiness_status, "NO_DATA")
...
test("getStudentM1V3 is cached across reads", async () => {
  route("/predict/m1v3/STU-A", 200, M1V3_BODY)
  await studentApi.getStudentM1V3()
  await studentApi.getStudentM1V3()
  const gets = getCalls("/predict/m1v3")
  assert.equal(gets.length, 1, "second read must hit the cache")
})

test("getStudentM1V3 maps 503 network/backend failure", async () => {
  route("/predict/m1v3/STU-A", 503, { detail: "down" })
  const result = await studentApi.getStudentM1V3()
  assert.equal(result.ok, false)
  if (!result.ok) assert.equal(result.error.status, 503)
})
