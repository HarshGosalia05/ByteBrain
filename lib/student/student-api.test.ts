...
const M1V3_BODY = {
  ...
  note: "Predicted end-sem marks are model estimates, not actual results. This prediction was generated by a model trained on synthetic data. Uncertainty is unavailable for this model.",
}
...
test("getStudentM1V3 fetches /predict/m1v3 with own student id and parses typed result", async () => {
  ...
  assert.equal(result.data.model_version, "1.0")
  assert.equal(result.data.readiness_status, "READY")
  assert.equal(result.data.subjects.length, 1)
  const subject = result.data.subjects[0]
  ...
  assert.equal(subject.input_features.attendance_percentage, 88.4)
  assert.ok(subject.predicted_end_sem_marks <= subject.target_max)
